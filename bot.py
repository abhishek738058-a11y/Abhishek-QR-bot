...
def callback_user_get_qr(call):
    bot.answer_callback_query(
        call.id,
        '⚠️ Filhaal koi naya QR live nahi hai! Channel join rakhein alert ke liye.',
        show_alert=True,
    )
...
def handle_invite_earn(message):
    ...
    text = (
        ...
        f'🔗 *Aapka Personal Referral Link:*\n`{ref_link}`\n\n👥 *Total Invited:* **{ref_count} Users**'
    )
    safe_send_message(message.chat.id, text, parse_mode='Markdown')
...
def process_admin_add_task(message):
    try:
        ...
        safe_send_message(
            target_id,
            f'🎉 *TASK APPROVED!*\nTask #{task_num} complete! **₹{reward:.2f}** credited!',
            parse_mode='Markdown',
        )
        cursor.execute(
        ...
        safe_send_message(
            message.chat.id,
            f'✅ Task #{task_num} Approved! User `{target_id}` credited with ₹{reward:.2f}',
            parse_mode='Markdown',
        )
    except Exception:
        safe_send_message(
...
def process_admin_qr_broadcast(message):
    ...
    cursor = conn.cursor()
    cursor.execute(
        'SELECT user_id FROM users WHERE notifications_enabled = 1 AND is_blocked = 0'
    )
